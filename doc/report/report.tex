...
The default parameter values are $\Delta \tau = 0$ and $p_d=p_h=0$. Parameter $a$ as varied by changing the artificial propagation delay $\Delta \tau$, and $R_e$ was varied by changing the probability of a data bit swap $p_d$. $C$ was kept at the highest value that yielded decent results, $C=\SI{115200}{\bit/\second}$.
...
-s, --size              Block size, in bytes
						(default is LL_MAX_SIZE, which is 4096)
