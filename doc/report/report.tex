...
\begin{itemize}
	\item Control field (C): Determine whether its a start or end control packet. Assumes the value \texttt{0x02} when its a start packet, and \texttt{0x03} when its an end packet.
	\item Type ($T_x$): Indicates if the parameter is a file size, \texttt{0x0}, or a file name, \texttt{0x01}.
	\item Lenght ($L_x$): Indicates the size in bytes of the parameter value.
	\item Value ($V_x$): The parameter value indicated by T and with the size indicated by L.
\end{itemize} 
...
\begin{figure}[H]
	...
	\begin{tabular}{m{6mm} m{108mm}}
		...
		\begin{tikzpicture}[->,>=stealth',node distance=2cm,initial text=$ $,]
			...
			\node[align=center, draw, minimum width=1.2cm, minimum height=1.2cm] at (8.4, 0) (pk) {$P_k$};
		\end{tikzpicture}
	\end{tabular}
	\caption{Structure of the data packet.}
...
\begin{itemize}
	\item Control field (C): Takes over the value \texttt{0x01}.
	\item Sequence number (N): Sequence number of the data chunk, which is a module of 255.
	\item $L_2$ and $L_1$: Second and first byte of the number representing the size in bytes of the data field.
	\item $P_1$ to $P_k$: K bytes of the file to be transmitted.
\end{itemize}
...
\begin{figure}[H]
	...
	\begin{tikzpicture}[->,>=stealth',node distance=1.4cm,initial text=$ $,]
		...
		\draw   (app) 		edge[align=center, left , bend right=88.5		] node{} (link)
			...
			;
	\end{tikzpicture}
	\caption{Receiver read order. The receiver gathers information about the file with the support of the control packets and reconstructs the original file with the data packets. 
