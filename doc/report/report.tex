...
\begin{itemize}
	...
	\item \texttt{./receiver <COM> <file>}, to receive \texttt{<file>} from port \texttt{<COM>}.
\end{itemize}

This module takes advantage of the link layer interface to transfer a file in chunks that should be: not too large, since the probability of having an error is increased, and as such implies large frames will often have to be retransmitted; and not too small, as the total amount of non-data bytes is proportional to the number of frames.
...
\begin{table}[H]
	...
	\begin{tabular}{l | p{12cm}}
		...
		\texttt{baud\_rate}      & Baud rate, the number of bits transmitted per second; defaults to 38400. Any number may be specified, but only a few specific baud rates can be used (see \texttt{termios(3)}); as such, the baud rate is rounded up to the next allowed baud rate, unless it is above the maximum baud rate, 230400. Is loaded on call to \texttt{llopen}. \\ \hline
		\texttt{timeout}         & Time the transmitter waits before retransmitting a message that the receiver did not acknowledge. It is measured in integer seconds, and should be greater than zero. By default it is 3 seconds, and can be set at any time. \\ \hline
		\texttt{retransmissions} & Number of times a frame will be retransmitted before giving up and terminating the connection. Defaults to 3, and can be set at any time. \\ \hline \hline
	\end{tabular}
...
First, start the receiver and connect it to \texttt{COM12} to read file \texttt{pinguim.gif} by calling

\begin{lstlisting}
./receiver 12 pinguim.gif
\end{lstlisting}
...
\begin{figure}[H]
	...
	\begin{minipage}{11cm}
		...
		\begin{tikzpicture}[->,>=stealth',node distance=2cm,initial text=$ $,]
			...
			\node[align=center, draw, minimum width=1.2cm, minimum height=1.2cm] at (4.8, 0) (d1) {D1};
		...
		\end{tikzpicture}
	\end{minipage}
	\caption{Structure of the S/U-frames (top) and I-frames (bottom). Each square represents a byte, each of type F (flag), A (address), C (control), BCC (block check character) or D (data).}
	\label{fig:frames}
\end{figure}
...
\begin{itemize}
	...
	\item Block check character (BCC): exclusive-or of a certain range of bytes (BCC and BCC1 are the exclusive-or of A and C, and BCC2 is the exclusive-or of all data D1, D2, ..., DN). Is used to check for bit swaps.
...
\begin{center}
	\begin{tikzpicture}[->,>=stealth',node distance=2.2cm,initial text=$ $,state/.style={circle, draw, minimum size=15mm}]
		...
		\node[align=center, state, below right of=A_RCV					] (C_SET_RCV) 		{C\\SET\\RCV};
		\node[align=center, state, below of=C_SET_RCV, accepting		] (STOP_DISC) 		{STOP\\DISC};
		...
		\draw	(START)			edge[right						]	node{FLAG}		(FLAG_RCV)
				...
				(A_RCV)			edge[							]	node{C\_SET}	(C_SET_RCV)
				(C_SET_RCV)		edge[align=center, loop right	]	node{$\Sigma$-\\ESC}	(C_SET_RCV)
				(C_SET_RCV)		edge[right						]	node{FLAG}		(STOP_DISC)
				;
...
\begin{center}
	\begin{tikzpicture}[->,>=stealth',node distance=2.2cm,initial text=$ $,state/.style={circle, draw, minimum size=15mm}]
		...
		\draw	(START)			edge[right						]	node{FLAG}		(FLAG_RCV)
				...
				(C_I_RCV)		edge[align=center, loop right	]	node{$\Sigma$-\\ESC}	(C_I_RCV)
...
\begin{center}
	\begin{tikzpicture}[->,>=stealth',node distance=2.2cm,initial text=$ $,state/.style={circle, draw, minimum size=15mm}]
		...
		\node[align=center, state, below of=D							] (D_ESC) 		{D\\ESC};
		\node[align=center, state, right=1.2cm of D_ESC, accepting		] (STOP) 		{STOP};
		...
		\draw	(START)			edge[right					]	node{FLAG}					(FLAG_RCV)
				...
				(D)				edge[loop left, looseness=2	]	node{$\Sigma-\text{ESC}$}	(D)
				(D)				edge[bend right=10, left	]	node{ESC}					(D_ESC)
				(D_ESC)			edge[bend right=10, right	]	node{$\Sigma$}				(D)
				(D)				edge[right					]	node{FLAG}					(STOP)

				(A_RCV)			edge[						]	node{C\_UNXP}	(C_UNXP_RCV)
				(C_UNXP_RCV)	edge[align=center, loop right, looseness=2]	node{$\Sigma-$\\ESC}	(C_UNXP_RCV)
				(C_UNXP_RCV)	edge[left					]	node{FLAG}		(STOP_RR)

				(A_RCV)			edge[right					]	node{C\_SET}	(C_SET_RCV)
				(C_SET_RCV)		edge[align=center, loop right, looseness=2]	node{$\Sigma-$\\ESC}	(C_SET_RCV)
