...
To test the program, run the following command: \texttt{make test}.
...
\begin{figure}[H]
	...
	\begin{subfigure}[t]{0.46\textwidth}
		...
		\end{tikzpicture}
	\end{subfigure}
	\caption{Efficiency $S$ plotted against $a$ (on the left) and $R_e$ (right). On the left, $S$ is determined for each point from the average $T$ of four repetitions} \label{fig:S_a_Re}
\end{figure}
...
\begin{equation*}
	S = \frac{1-R_e}{1 + 2a}
\end{equation*}
Qualitatively, the graphs in Figure \ref{fig:S_a_Re} show $S$ depends inversely on $a$ and linearly on $R_e$, in agreement with the predictions. To formally evaluate the validity of this expression, parameters $a$ and $R_e$ were simultaneously varied to obtain the data in the graph of Figure \ref{fig:S_aRe}. Additionally, we calculated the RMSE between the experimentally obtained efficiency and the theoretically predicted efficiency, which is 0.029860; this is possibly caused by the fact there is a linear relation $S=0.916422\,S_{pred} - 0.003571$ with correlation coefficient $r^2 = 0.999707$, where the slope differs from $1$ because of some sort of additional work that causes $S$ to be smaller than predicted by around $8\%$.
...
% TODO (still needs changes)

\section{Conclusion} \label{sec:Conclusion}

The project aimed in implementing a reliable protocol to transfer a file between two computers connected.
It consisted in a link layer and an application layer, the two being independent of each other. 
The main objective was achieved and the protocol was able to withstand several tests.

The group had the opportunity to put into practice the theoretical content learned, and to develop these teachings even further by doing analysis and debating ways to build the program.
