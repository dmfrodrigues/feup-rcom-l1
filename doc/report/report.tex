...
To formally evaluate the validity of this expression, parameters $a$ and $R_e$ were simultaneously varied to obtain the data in the graph of Figure \ref{fig:S_aRe}.
We also calculated the RMSE between the experimentally obtained efficiency and the theoretically predicted efficiency, which is 0.029860;
this is possibly caused by the fact there is a linear relation $S=0.916512\,S_{pred} - 0.003564$ with very high correlation coefficient $r^2 = 0.999707$, where the slope differs from $1$ possibly due to header bytes and driver overhead that causes $S$ to be smaller than predicted by around $8\%$.
